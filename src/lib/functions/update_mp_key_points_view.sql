...
-- Add GIN index for speaker_name pattern matching
CREATE INDEX idx_mp_key_points_speaker_name_gin ON public.mp_key_points 
  USING gin (speaker_name gin_trgm_ops);

-- Add composite index for speaker_name + date for sorted searches
CREATE INDEX idx_mp_key_points_speaker_name_date ON public.mp_key_points 
  (speaker_name, debate_date DESC);
