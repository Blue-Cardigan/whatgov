  ...
  return (
    <>
      <div className={cn(
        "w-full grid",
        isEnabled 
          ? "grid-cols-6 md:grid-cols-1"
          : "grid-cols-1 md:grid-cols-1",
        "md:gap-y-6 px-4 py-3 md:pt-6 md:px-0"
      )}>
        {/* MP Filter Button */}
        <button
          className={cn(
        ...
        {/* Only show array filters if enabled (user has subscription) */}
        {isEnabled && filterItems
          .filter(filter => filter.type === 'array')  // Only show array filters here
          .map((filter) => (
            <Popover 
              key={filter.id} 
              open={open[filter.id]}
